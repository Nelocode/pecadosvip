...
Cuando `output: 'standalone'` está activo, el build genera además `dist/standalone/server.js`. El paso `standalone:prepare` incorpora de forma desreferenciada los peers runtime que Vinext `1.0.0-beta.3` omite con la configuración Cloudflare actual y elimina sourcemaps de dependencias del artefacto. `release:artifact` valida por separado el `dist` worker y el standalone Node, incluidos el paquete Vinext y su `prod-server`; `smoke:production` arranca exactamente `node dist/standalone/server.js` en loopback. El `Dockerfile` multi-stage copia solo ese resultado a una imagen Node no-root con etiqueta de versión exacta. La base no está fijada por digest inmutable y debe registrarse el digest real al construir la imagen candidata.
...
El workbench permite crear, editar, duplicar, cambiar estado o disponibilidad, registrar evidencia/aprobación y ordenar o asociar medios, respetando revisión optimista, roles y protección contra replay. La interfaz acepta imágenes JPEG, PNG o WebP de hasta 5 MiB, valida firma y hash y conserva metadatos y eventos de auditoría. Archivar/restaurar perfiles o medios no elimina físicamente sus archivos. No hay transformación responsive, optimización, antivirus, CDN ni publicación automática al sitio público.

Solo debe existir **un proceso escritor** para un directorio de datos. La serialización protege operaciones dentro del mismo proceso, no entre procesos o equipos. Detén el workbench con `Ctrl+C` antes de copiar, respaldar o restaurar sus archivos.
